...
def train(model, device, loader_trn, optimizer, epoch, log_interval, dry_run=False, epochs=None, st_text=None, bar=None):
    ...
    for batch_idx, (data, target) in enumerate(loader_trn):
        ...
        optimizer.zero_grad()
        output = model(data)
        loss = F.nll_loss(output, target) #crossentropy
        loss.backward()
        optimizer.step()
...
def train_fmnist():
    ...
    dts_trn = datasets.FashionMNIST('data', train=True, download=True,
                             transform=transform)
    dts_val = datasets.FashionMNIST('data', train=False,
                             transform=transform)
    ...
    if args.save_model:
        torch.save(model.to('cpu').state_dict(), "../models/fmnist_cnn.pt")


def finetune_fmnist(model: Net,
                   model_filename: str,
                   dts_trn: FashionMNIST,
                   dts_val: FashionMNIST,
...
if __name__ == '__main__':
    train_fmnist()
    # finetune_fmnist(model= Net, model_filename='fmnist_cnn.pt',dts_trn=FashionMNIST, dts_val=FashionMNIST, trn_batch_size=32, options= 'Fine-tune on fc2 and fc1', gamma='0.96', lr='0.001')
